...
def generateInfo(field_references: Tuple, lang: str) -> str:
    ...
    FTYPE_ARR = ["int", "long", "bool", "char", "double"]
    ACCESS_MODIFIER_ARR = ["private", "protected", "public"]
    ...
    field_modifier = ACCESS_MODIFIER_ARR[field_references[1]]
    is_static = field_references[2]
    field_access = ACCESS_TYPE_ARR[field_references[3]]
    ...
    if is_static:
        subfield_frag += ""
    elif subclass_rel == "outer" and (field_modifier == "private" or field_access == "class_this"):
        subfield_frag += " = " + getPrimVal(field_type)
    elif not is_static and field_access == "static":
        subfield_frag += " = " + getPrimVal(field_type)
    else:
    ...
    subfield_decl = field_type + " " 
    subfield_decl += "static " if is_static else "" 
    subfield_decl += subfield_frag
    

    sub_cls_code = subcls_decl
    sub_cls_code += field_modifier + ":\n"
...
def twoClsFldCldGenerate(input_file: str, template_file: str, input_size: int, language: str, output_dir: str, single: bool):
    if language == "cpp":
        extension: str = ".cpp"
    elif language == "cuda":
        extension: str = ".cu"
