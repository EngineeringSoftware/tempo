...
def generateMain(
    ...
    if language == "cpp":
        ...
        for function in functions:
            program.insert(kernel_definition + 1, "    printf(\"Function " + function.name + " returned %d\\n\", x);")
            program.insert(kernel_definition + 1, "    x = " + function.get_call(True))
        program.insert(kernel_definition + 1, "    int x;")
    ...
    for argument in initialized_arguments:
        program.insert(kernel_definition + 1, "    " + argument)
...
def cgGenerate(input_file: str, template_file: str, struct_template: str, input_size: int, language: str, output_dir: str, single: bool):
    if language == "cpp":
        extension: str = ".cpp"
    ...
    if single:
        ...
        for idx, cg in enumerate(programs):
            ...
            generateMain(functions, variables, cg, input_size, STRUCTS_HEADER_FILE, STRUCT_NAME, FUNCTIONS_HEADER_FILE, NUM_BLOCKS, NUM_THREADS, list(programs), template_file, language, output_file)