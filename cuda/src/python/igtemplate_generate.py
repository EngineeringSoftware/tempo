...
class Class:
    ...
    def __init__(self, method_modifiers: List[str], field_modifiers: List[str], name: str, class_id: int):
        ...
        self.template_args = 0

    def set_template(self, template_args):
    ...
    def get_definition(self) -> str:
        ...
        if self.has_template:
            definition += "template<" + ", ".join([f"typename T{i}" for i in range(self.template_args)])+">\n"
        definition += "class " + self.name
        if (len(self.parents) > 0):
            definition += " : "
            for parent in self.parents:
                parent_decl = parent.name + "<" +", ".join(["void" for i in range(parent.template_args)]) + ">" if parent.has_template else parent.name
                definition += "public " + parent_decl + ", "
...
def generateClasses(inheritance_graph: Tuple, method_modifiers: List[str], ig_size: int, classes: List[Class]) -> List[Class]:
    ...
    for i in range(ig_size):
        has_template = inheritance_graph[curr_ind]
        template_args = inheritance_graph[curr_ind + 1]

        if has_template == 1:
            classes[i].set_template(template_args)
...
def generateMain(
    ...
    for i in range(len(classes)):
        ...
        cls_decl = classes[i].name + "<" +", ".join(["void" for i in range(classes[i].template_args)]) + ">" if classes[i].has_template else classes[i].name
        program.insert(kernel_definition + 1, "    " + cls_decl + " o" + str(i) + ";")
...
def igTemplateGenerate(input_file: str, template_file: str, input_size: int, language: str, output_dir: str, single: bool, n: int):
    if language == "cpp":
        extension: str = ".cpp"
