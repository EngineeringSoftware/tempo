...
class Constructor:
    member_init_list: bool
    parameters: List[str]
    fields: List[Field]
    body: List[str]

    def __init__(self, class_name: str, fields: List[Field], member_init_list: bool, option: str):
        self.member_init_list = member_init_list
    ...
    def get_constructor(self) -> str:
        body = self.class_name
        if self.member_init_list:
            if self.option == "default":
                body += "(): "
                body = self.fields[0].name +"{0}"
                for i in range(1, len(self.fields)):
                    body += ", " + self.fields[i].name + "{0}"
            elif self.option == "normal":
                body += "(" + ",".join([f.type_name + " " + f.name for f in self.fields]) + "): "
                body += self.fields[0].name +"{"+self.fields[0].name+"}"
                for i in range(1, len(self.fields)):
                    body += ", " + self.fields[i].name + "{"+self.fields[i].name+"}"
            elif self.option == "copy":
                body += "(const "+self.class_name+"& X): " 
                body += self.fields[0].name +"{X."+self.fields[0].name+"}"
                for i in range(1, len(self.fields)):
                    body += ", " + self.fields[i].name + "{X."+self.fields[i].name+"}"
            body += " {}"
        else:
            if self.option == "default":
                body += "() {\n"
                ...
                body += "("+ ",".join([f.type_name + " " + f.name for f in self.fields]) +") {\n"
                for i in range(len(self.fields)):
                    body += "this->"+self.fields[i].name + " = "+self.fields[i].name+";\n"
                body+="}\n"
            elif self.option == "copy":
                body += "("+"const "+self.class_name+"& X" +") {\n"
                for i in range(len(self.fields)):
                    body += "this->"+self.fields[i].name + " = X."+self.fields[i].name+";\n"
                body+="}"
        return body
...
class Class:
    ...
    def __init__(self, constructors: List[Constructor], method_modifiers: List[str], field_modifiers: List[str], name: str, class_id: int):
        self.name = name
    ...
    def get_definition(self, language: str) -> str:
        ...
        for c in self.constructors:
            device = "__device__ " if language == "cuda" else ""
            definition += device + "    " + c.get_constructor() + "\n"
...
def generateClasses(inheritance_graph: Tuple, method_modifiers: List[str], ig_size: int, classes: List[Class]) -> List[Class]:
    ...
    CONSTRUCTOR_TYPES = ["normal", "copy"]

    for i in range(ig_size):
        constructor_type = CONSTRUCTOR_TYPES[inheritance_graph[curr_ind]]
        is_member_initializer_list = (inheritance_graph[curr_ind + 1] == 1)
        
        curr_ind += 2
        classes[i].constructors.append(Constructor(classes[i].name, classes[i].fields, is_member_initializer_list, constructor_type))
        classes[i].constructors.append(Constructor(classes[i].name, classes[i].fields, False, "default"))
...
def generateMain(
    ...
    for c in classes:
        program.insert(1, c.get_definition(language))
...
def igConstructorGenerate(input_file: str, template_file: str, input_size: int, language: str, output_dir: str, single: bool):
    if language == "cpp":
        extension: str = ".cpp"
    ...
    if single:
        ...
        for idx, ig in enumerate(programs):
            ...
            generateMain(classes, ig, input_size, METHOD_MODIFIERS, NUM_BLOCKS, NUM_THREADS, template_file, language, output_file, single)