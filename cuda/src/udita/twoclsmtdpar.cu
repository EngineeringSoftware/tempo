...
void printFss(int8_t *dci_arr, int valid_dci, int struct_size) {
    printf("Programs:\n");
...
__device__ void dciGenerate(DCI *dci) {
    ...
    dci->supercls_rel = (int8_t) _choice(0, MAX_SUPERCLASS_REL - 1);}

__global__ void twoClsMtdParUdita(int32_t bck_active, int32_t size) {
    unsigned int idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx >= bck_active) {
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    explore((void (*)(...)) twoClsMtdParUdita, args, 1);
    // EXPLORE(twoClsMtdParUdita<<<starting_blocks, starting_threads>>>(active_threads, size));
    ...
    printFss(dci_arr, valid_dci, STRUCT_FIELDS);
