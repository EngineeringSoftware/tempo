...
__device__ void igAdjMatrix(Env *env, IG *ig) {
    ...
    for (int32_t i = 0; i < op->size; i++) {
        Node *n = op->getObject(op, i);
        if (n != NULL) {
            adjacency_matrices[curr_ind] = n->has_template;
            adjacency_matrices[curr_ind + 1] = n->template_args;
        }
...
//             n->has_template = _choice(0, 1);
//             if (n->has_template == 1) {
//                 n->template_args = _choice(1, 3);
//             }
//             else {
//                 n->template_args = 0;
//             }
