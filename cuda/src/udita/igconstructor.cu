...
#include "igconstructor.h"
#include "../explore.h"

ImpObjPool(Node);

#define OUTPUT_BUFFER 1000000000
#define STRUCT_FIELDS 2
...
__device__ void igAdjMatrix(Env *env, IG *ig) {
    ...
    for (int32_t i = 0; i < op->size; i++) {
        Node *n = op->getObject(op, i);
        if (n != NULL) {
            adjacency_matrices[curr_ind] = n->constructor_type;
            adjacency_matrices[curr_ind + 1] = n->is_member_initializer_list;
        }
...
__device__ void igGenerate(Env *env, IG *ig) {
    ...
    n->visited = 0;
    n->num_of_children = 2;
    n->constructor_type = _choice(0, 1);
    n->is_member_initializer_list = _choice(0, 1);
